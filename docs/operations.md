...
`beacon-rel-budget-dock` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for budget scenarios with seeded input scenarios, deterministic summary checks, and single-node deterministic mode.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.
